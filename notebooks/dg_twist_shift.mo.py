...
def _imports():
  ...
  import marimo as mo
  return (
    glob, re, subprocess, Path,
    matplotlib, plt, patches, Line2D, np, pg, mo,
  )
...
def _controls(mo):
  ...
  boundary = mo.ui.dropdown(
    options={"Lower z": "lower", "Upper z": "upper"},
    value="Lower z",
    label="Boundary",
  )
...
def _compile_run(mo, re, subprocess, Path,
  ...
  _content = re.sub(
    r'(int Nx\s*=\s*)\d+(\s*;)',
    rf'\g<1>{int(nx.value)}\g<2>', _content)
  _content = re.sub(
    r'(int Ny\s*=\s*)\d+(\s*;)',
    rf'\g<1>{int(ny.value)}\g<2>', _content)
  ...
  if _make.returncode != 0:
    _msg = _make.stdout + "\n" + _make.stderr
    mo.stop(True, mo.callout(mo.md(f"**make failed:**\n```\n{_msg}\n```"), kind="danger"))
  ...
  if _run.returncode != 0:
    _msg = _run.stdout + "\n" + _run.stderr
    mo.stop(True, mo.callout(mo.md(f"**Run failed:**\n```\n{_msg}\n```"), kind="danger"))
...
def _donor_plot(mo, result, getRawGrid, getRawData, np, plt, patches, Line2D):
  ...
  for _ix in range(_Nx):
    for _iy in range(_Ny):
      _ax.add_patch(patches.Rectangle(
        (_x_nodes[_ix], _y_nodes[_iy]), _dx_cells[_ix], _dy,
        lw=0.4, edgecolor='#aaaaaa', facecolor='white', zorder=1))
  ...
  for _ix in range(_Nx):
    ...
    for _iy in _donor_iys:
      _ax.add_patch(patches.Rectangle(
        (_x_lo, _y_nodes[_iy]), _dx_cells[_ix], _dy,
        alpha=0.38, facecolor='royalblue', edgecolor='none', zorder=2))
  ...
  def _plot_wrapped_line(y_raw, ls, lbl):
    ...
    _ax.plot(_x_dense, np.where(mask, np.nan, y),
        color='navy', lw=1.8, ls=ls, zorder=5, label=lbl)
  ...
  # Wrap points
  for _ix in range(_Nx):
    _x_lo = _x_nodes[_ix];  _x_up = _x_nodes[_ix + 1]
    _S_lo = _S_lo_cells[_ix];  _S_up = _S_up_cells[_ix]
    if _S_lo == _S_up:
      continue
    ...
    for _n_wrap in range(_n_lo, _n_up + 1):
      ...
      if _x_lo < _x_wrap < _x_up:
        _ax.axvline(_x_wrap, color='limegreen', lw=1.2, ls=':', zorder=4, alpha=0.85)

  # ΔS/dy annotations
  for _ix, _dS in enumerate(_dS_cells):
    _color = 'red' if _dS >= _Ly else 'black'
    _ax.text(_x_centers[_ix], _y_min - 0.06 * _Ly, f'{_dS/_dy:.1f}',
        ha='center', va='top', fontsize=6, color=_color)
  _ax.text(_x_min - 0.004 * _Lx, _y_min - 0.06 * _Ly,
      r'$\Delta S/dy$:', ha='right', va='top', fontsize=7, color='black')

  # Legend
  _legend_elems = [
    patches.Patch(fc='royalblue', alpha=0.4, label='donor cells'),
    Line2D([0], [0], color='navy', lw=1.8, label=r'$y_{\rm tar,lo} - S(x)$'),
    Line2D([0], [0], color='navy', lw=1.8, ls='--', label=r'$y_{\rm tar,up} - S(x)$'),
    Line2D([0], [0], color='limegreen', lw=1.2, ls=':', label=r'wrap point: $S(x)=n\,L_y$'),
  ]
  _ax.legend(handles=_legend_elems, loc='upper center', ncols=2, fontsize=9,
       framealpha=0.92, frameon=True, bbox_to_anchor=(0.5, 1.0))
...
def _field_plot(mo, pg, np, plt, result, species, quantity, comp_ui, boundary):
  ...
  _pcm = _ax.pcolormesh(
    _x_out[0], _x_out[1], _field.T,
    shading='flat', cmap='inferno')
  plt.colorbar(_pcm, ax=_ax, label=f"{species.value}_{quantity.value} [comp {_comp}]")
...
def _ts_lib(mo, gkylsoft_ui, run_btn):
  ...
  _lc.gkyl_sub_range_init.argtypes = [_c_vp, _c_vp,
    ctypes.POINTER(_c_i), ctypes.POINTER(_c_i)]
  _lc.gkyl_sub_range_init.restype = None

  _lib_gk.gkyl_bc_twistshift_new.argtypes = [_c_i, _c_i, _c_i, _c_i, _c_i, _c_vp,
    ctypes.POINTER(_c_i), _c_vp, _c_vp, _c_vp, _c_vp, _c_vp, _c_i, _c_b,  ]
...
@app.cell
def _ts_plot(mo, ctypes, ts_lib, result, species, quantity, comp_ui, boundary, pg, np, plt):
  if result is None or not result[0]:
    mo.stop(True, mo.md("Run the simulation first."))
  ...
  _basis_ptr = _lc.gkyl_cart_modal_serendip_new(ctypes.c_int(3), ctypes.c_int(_poly_order))
  ...
  _f_arr = _lc.gkyl_array_new_from_buff(
    2, ctypes.c_size_t(_ncomp), ctypes.c_size_t((_Nx + 2) * (_Ny + 2) * (_Nz + 2)),
  ...
  _up = _lib_gk.gkyl_bc_twistshift_new(
    2, 1, 0, _edge, 3,
    _bc_range, _ghost, _basis_ptr, _grid_ptr,
    None, None, _shift_arr, _shift_poly_order, False)
  ...
  _fig, _ax = plt.subplots(figsize=(9, 5.5))
  _ax.set_aspect('equal')
  _pcm = _ax.pcolormesh(_x_interp[0], _x_interp[1], _ghost_field.T, shading='flat', cmap='inferno')
  plt.colorbar(_pcm, ax=_ax, label=f"{species.value}_{quantity.value} ghost [comp {_comp}]")
  _ax.set_xlabel('x  (m)', fontsize=12)
  _ax.set_ylabel('y  (m)', fontsize=12)
  _bnd_label = "lower" if _bnd == "lower" else "upper"
  _ax.set_title(
