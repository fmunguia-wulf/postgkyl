...
# The ISA target libg0core.so's kernels were actually compiled with -- read
# back from gkeyll's own config.mak (written by ./configure) rather than
# re-deriving it, so this can never drift from what build_gkeyll.sh picked.
BUILD_ARCH_FLAGS=$(sed -n 's/^ARCH_FLAGS=//p' "${GKEYLL_DIR}/config.mak" 2>/dev/null | head -1)
