...
- **`api.py`** — **generated, do not edit by hand.** It defines `PgkylSession`
...
- **`_api_gen.py`** — the generator that produces `api.py`.

## Updating `api.py`

The pgkyl click commands are the single source of truth: each command and option already carries a name, type, default, and help string. `_api_gen.py` introspects that metadata and emits one typed method per command.

Whenever you add, remove, or change a pgkyl command or its options, regenerate:

```bash
python -m postgkyl.api._api_gen          # rewrite api.py
python -m postgkyl.api._api_gen --check  # exit non-zero if api.py is stale (for CI/tests)
```
