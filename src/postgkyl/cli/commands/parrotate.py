...
@tag_option(default="rotarraypar")
@label_option(default="rotarraypar")
@click.pass_context
def command(ctx, array_tag, rotator_tag, coords, tag, label) -> None:
  """Rotate a three-component array parallel to a rotator's unit vector."""
  array = find_by_tag(ctx, array_tag)
  rotator = find_by_tag(ctx, rotator_tag)
  result = pg.diagnostics.rotations.parrotate(array, rotator, coords=coords,
      tag=tag, label=label)
