...
def resolve_axis_labels(*, xlabel: str | None, ylabel: str | None,
    zlabel: str | None, clabel: str, num_dims: int,
  ...
  if zscale != 1.0:
    clabel = (rf"{clabel:s} $\times$ {zscale:.3e}" if clabel
...
def prep_plot_data(data: "GDataState", *, xlabel: str | None = None,
    ylabel: str | None = None, clabel: str = "",
    xshift: float = 0.0, yshift: float = 0.0, zshift: float = 0.0,
    xscale: float = 1.0, yscale: float = 1.0, zscale: float = 1.0) -> PlotPanel:
  ...
  num_dims = len(grid)
  xlabel, ylabel, _zlabel, clabel = resolve_axis_labels(
      xlabel=xlabel, ylabel=ylabel, zlabel="", clabel=clabel,
