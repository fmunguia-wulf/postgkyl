...
This is the module's dedicated home post-relocation (``ffi/rep.py`` ->
...
layers" section for why representation changes live in ``dg``, not ``ffi``.
...
def _linear_field(basis_type, ndim, poly_order, cells, nfields=1):
  ...
  return ffi.GkylArray.from_numpy(vals), nb
...
def test_apply_pointwise_sqrt_matches_numpy_after_interp():
  ...
  ndim, poly_order = 1, 1
  arr, nb = _linear_field("serendipity", ndim, poly_order, [4])
  arr = ffi.kernels.shiftc(arr, 5.0, 0)  # keep the field positive for sqrt
  out = dg.rep.apply_pointwise(ndim=ndim, poly_order=poly_order,
      basis_type="serendipity", arr=arr, fn=np.sqrt, num_quad=poly_order + 1)
  grid, direct_vals = dg.interpolate(arr.view(), [np.linspace(0, 4, 5)],
      poly_order=poly_order, basis_type="serendipity")
