...
@pytest.mark.parametrize("basis_type,ndim,poly_order", [
    ("serendipity", 1, 0), ("serendipity", 1, 1), ("serendipity", 1, 2),
    ("serendipity", 1, 3), ("serendipity", 2, 0), ("serendipity", 2, 1),
    ("serendipity", 2, 2), ("serendipity", 2, 3),
    ("tensor", 1, 2), ("tensor", 2, 2), ("tensor", 3, 1),
])
...
@pytest.mark.parametrize("basis_type,ndim,poly_order", [
    ("serendipity", 5, 2), ("serendipity", 6, 1), ("tensor", 3, 2),
])
...
@pytest.mark.parametrize("basis_type,ndim,expected_num_basis", [
    ("hybrid", 2, 6), ("hybrid", 3, 12), ("hybrid", 4, 24),
    ("gkhybrid", 2, 6), ("gkhybrid", 3, 12), ("gkhybrid", 4, 24),
    ("gkhybrid", 5, 48),
])
def test_hybrid_num_basis_matches_gkeyll_kernel_tables(
    basis_type, ndim, expected_num_basis):
...
def test_hybrid_unsupported_combinations_raise_cleanly(
...
@pytest.mark.parametrize("basis_type,ndim", [
    ("hybrid", 2), ("hybrid", 3), ("gkhybrid", 2), ("gkhybrid", 3),
    ("gkhybrid", 4),
])
