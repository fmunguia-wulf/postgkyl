...
FIELD_FILES = sorted(glob.glob(os.path.join(DATA, "rt_gk_tcv_iwl_1x2v_p1-*.gkyl")))
GENERATED_FILES = sorted(glob.glob(os.path.join(DATA, "generated", "*.gkyl")))
...
def test_write_field_round_trips_bit_exactly(tmp_path):
  ...
  grid = {"lower": np.array([0.0, -1.0]), "upper": np.array([2.0, 1.0]),
          "cells": np.array([4, 3])}
...
def test_write_field_is_readable_by_the_pure_python_reader(tmp_path):
  ...
  grid = {"lower": np.array([0.0]), "upper": np.array([5.0]), "cells": np.array([5])}
