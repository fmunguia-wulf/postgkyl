...
class TestFitFunctions:
  ...
  def test_quadratic_evaluation(self):
    ...
    np.testing.assert_allclose(fitmod.quadratic(x, 1.0, -2.0, 1.0), [1.0, 0.0, 1.0, 4.0])
  # end
  ...
  def test_power_evaluation(self):
    ...
    np.testing.assert_allclose(fitmod.power(x, 2.0, 3.0, 1.0), [3.0, 17.0, 129.0])
  # end

  def test_sinusoid_evaluation(self):
    x = np.array([0.0, np.pi / 2])
    np.testing.assert_allclose(fitmod.sinusoid(x, 1.0, 1.0, 0.0, 0.5), [0.5, 1.5], atol=1e-14)
  # end
...
class TestRPN:
  ...
  def test_rpn_plane_2d_recovers_params(self):
    ...
    params, _, R2 = fitmod.fit(xdata, y, "a x * b y * + c +", p0=[1.0, 1.0, 0.0])
...
class TestFit2D:
  ...
  def test_plane_noisy_data_high_R2(self):
    ...
    zdata = fitmod.plane(xdata, 2.0, -1.5, 0.5) + rng.normal(0, 0.05, xdata.shape[1])
...
class TestAutoGuess:
  ...
  def test_plane_guess_is_reasonable(self):
    X, Y = np.meshgrid(np.linspace(0, 5, 20), np.linspace(0, 3, 15), indexing="ij")
    xdata = np.array([X.flatten(), Y.flatten()])
    y = 2.0 * X.flatten() - 1.5 * Y.flatten() + 0.5
  ...
  def test_quadratic2d_guess_is_reasonable(self):
    X, Y = np.meshgrid(np.linspace(0, 4, 15), np.linspace(0, 3, 12), indexing="ij")
    xdata = np.array([X.flatten(), Y.flatten()])
    true_params = [0.3, 0.2, -0.1, 1.0, -0.5, 2.0]
