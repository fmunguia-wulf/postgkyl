...
def _synthetic_map(coeffs, lower, upper, cells, *, basis_type="serendipity",
  ...
  d.ctx.update(basis_type=basis_type, poly_order=poly_order,
      value_form="modal", cells=np.asarray(cells, dtype=np.int64))
...
def _linear_map_dataset(a00, a01, a10, a11, *, lower=(0.0, 0.0),
    upper=(4.0, 4.0), cells=(8, 8), poly_order=1):
  ...
  target = GDataState()
  target.push(
      [np.linspace(lower[i], upper[i], cells[i] + 1) for i in range(2)],
...
class TestIntegrateCurvilinear:
  def test_rotation_preserves_area(self):
    ...
    mapped, x, _y = _linear_map_dataset(c, -s, s, c, lower=lower, upper=upper,
        cells=cells)
  ...
  def test_shear_area_matches_determinant(self):
    ...
    mapped, x, _y = _linear_map_dataset(a00, a01, a10, a11, lower=lower,
