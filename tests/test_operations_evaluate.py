...
def test_min_max_mean():
  ...
  assert operations.evaluate("f min", a).get_values().flatten()[0] == pytest.approx(1.0)
  assert operations.evaluate("f max", a).get_values().flatten()[0] == pytest.approx(4.0)
  assert operations.evaluate("f mean", a).get_values().flatten()[0] == pytest.approx(2.5)
# end
