...
def _volume_3d(fn=lambda x, y, z: x + y + z, n=4):
  grid = [np.linspace(0.0, 1.0, n), np.linspace(0.0, 1.0, n), np.linspace(0.0, 1.0, n)]
...
class TestPlotlySurface2D:
  ...
  def test_scale_and_shift_apply_to_surface_coordinates_and_height(self):
    ...
    n, m = 4, 5
    fig = plotly(_surface_2d(n, m), xscale=2.0, xshift=1.0,
        yscale=3.0, yshift=0.5, zscale=2.0, zshift=1.0)
...
class TestPlotly3DVolume:
  ...
  def test_explicit_ranges_and_surface_count_override(self):
    fig = plotly(_volume_3d(), xrange=(0.2, 0.8), yrange=(0.1, 0.9),
        zrange=(0.3, 0.7), surface_count=12)
  ...
  def test_logc_converts_linear_clim_to_log_space(self):
    fig = plotly(_volume_3d(fn=lambda x, y, z: 1.0e-2 + x + y + z),
        logc=True, cmin=1.0e-20, cmax=1.0e-2)
  ...
  def test_scale_and_shift_apply_to_volume_coordinates(self):
    fig = plotly(_volume_3d(), xscale=2.0, xshift=1.0,
  ...
  def test_cylindrical_to_cartesian_conversion(self):
    ...
    np.testing.assert_allclose(fig.layout.scene.xaxis.range, (-1.0, 1.0), atol=1e-12)
    np.testing.assert_allclose(fig.layout.scene.yaxis.range, (-1.0, 1.0), atol=1e-12)
    np.testing.assert_allclose(fig.layout.scene.zaxis.range, (-0.5, 0.5), atol=1e-12)
  # end
# end
...
class TestPlotly3DScatter:
  def test_scatter_trace_basic_properties(self):
    fig = plotly(_volume_3d(), scatter=True, marker_radius=3.0,
        markerstyle="square", cmin=0.2, cmax=2.8)
  ...
  def test_log_opacity_ramp(self):
    fig = plotly(_volume_3d(), scatter=True,
        scatter_opacity_range=(0.01, 1.0), scatter_opacity_log=True)
    colorscale = fig.data[0].marker.colorscale
    alphas = np.array([float(c.split(",")[-1].rstrip(")")) for _, c in colorscale])
...
class TestSaveRotatingPlotlyFigure:
  ...
  def test_bad_extension_raises(self):
    with pytest.raises(ValueError, match=r"\.gif, \.mp4, or \.html"):
      save_rotating_plotly_figure(self._scene_fig(), "out.bogus", 0.0, 10,
          60.0, 2.0)
    # end
  # end
  ...
  def test_nonpositive_fps_raises(self):
    with pytest.raises(ValueError, match="fps"):
      save_rotating_plotly_figure(self._scene_fig(), "out.gif", 0.0, 0,
          60.0, 2.0)
    # end
  # end
  ...
  def test_nonpositive_rotation_period_raises(self):
    with pytest.raises(ValueError, match="rotation_period"):
      save_rotating_plotly_figure(self._scene_fig(), "out.gif", 0.0, 10,
          60.0, 0.0)
    # end
  # end
  ...
  def test_html_export_zero_rotation_period_omits_script(self, tmp_path):
    ...
    out = tmp_path / "out.html"
    save_rotating_plotly_figure(self._scene_fig(), str(out),
...
class TestPlotlyAnimate:
  def test_builds_frames_and_controls(self):
    ...
    fig = plotly_animate([_state(grid, values0), _state(grid, values1)],
        frame_duration=40)
    assert isinstance(fig, go.Figure)
    assert isinstance(fig.data[0], go.Surface)
