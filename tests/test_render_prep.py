...
class TestResolveAxisLabels:
  def test_defaults_for_2d(self):
    xl, yl, zl, cl = resolve_axis_labels(xlabel=None, ylabel=None, zlabel=None,
        clabel="", num_dims=2)
    assert xl == r"$z_0$"
    assert yl == r"$z_1$"
  # end
  ...
  def test_custom_labels_pass_through(self):
    xl, yl, zl, cl = resolve_axis_labels(xlabel="myX", ylabel="myY",
        zlabel="myZ", clabel="myC", num_dims=2, zscale=2.0)
    assert xl == "myX"
    assert yl == "myY"
    assert "2.00" in cl
  # end
  ...
  def test_clabel_annotated_with_zscale(self):
    _, _, _, cl = resolve_axis_labels(xlabel=None, ylabel=None, zlabel=None,
        clabel="density", num_dims=2, zscale=3.0)
    assert cl == r"density $\times$ 3.000e+00"
  # end

  def test_clabel_zscale_with_no_base_label(self):
    _, _, _, cl = resolve_axis_labels(xlabel=None, ylabel=None, zlabel=None,
        clabel="", num_dims=2, zscale=3.0)
