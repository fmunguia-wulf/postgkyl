"""Comprehensive tests for tools.calculus — integrate function."""
...
class TestIntegrate1D:
    def test_uniform_ones_integrates_to_domain_length(self):
        ...
        _, out = tools.integrate(d, axis=0)
    ...
    def test_none_axis_integrates_all(self):
        grid = [np.linspace(0.0, 1.0, 6)]
        d = _make(grid, np.ones((5, 1)))
        _, out = tools.integrate(d, axis=None)
        np.testing.assert_allclose(out.flat[0], 1.0, rtol=1e-12)

    def test_overwrite_updates_data_in_place(self):
        grid = [np.linspace(0.0, 1.0, 6)]
        d = _make(grid, np.ones((5, 1)))
        tools.integrate(d, axis=0, overwrite=True)
        assert d.get_values().flat[0] == pytest.approx(1.0)

    def test_stack_parameter_triggers_overwrite_with_deprecation(self, capsys):
        grid = [np.linspace(0.0, 1.0, 6)]
        d = _make(grid, np.ones((5, 1)))
        tools.integrate(d, axis=0, stack=True)
...
class TestIntegrate2D:
    ...
    def test_integrate_axis1_only(self):
        ...
        d = _make(grid, np.ones((3, 4, 1)))
        _, out = tools.integrate(d, axis=1)
    ...
    def test_comma_separated_string_axes(self):
        ...
        d = _make(grid, np.ones((5, 4, 1)))
        _, out = tools.integrate(d, axis="0,1")
        np.testing.assert_allclose(out.flat[0], 2.0, rtol=1e-12)

    def test_nonuniform_grid(self):
        # Nonuniform 1D grid — integral of 1 is total length
        x = np.array([0.0, 0.1, 0.4, 1.0])
        grid = [x]
        d = _make(grid, np.ones((3, 1)))
        _, out = tools.integrate(d, axis=0)
        np.testing.assert_allclose(out.flat[0], 1.0, rtol=1e-12)
...
class TestIntegrateCellCentered:
    def test_cell_centered_grid(self):
        # When len(coord) == values.shape[d], a last element is appended to dz
        x_cc = np.linspace(0.1, 0.9, 5)  # 5 cell centers
        grid = [x_cc]
        d = _make(grid, np.ones((5, 1)))
        # dz for cell centers: dx = 0.2 uniform, last repeated
        _, out = tools.integrate(d, axis=0)
        # 5 * 0.2 = 1.0
        np.testing.assert_allclose(out.flat[0], 1.0, rtol=1e-12)
    ...
    def test_single_cell_axis_uses_mean(self):
        ...
        d = _make(grid, np.ones((1, 3, 1)))
        _, out = tools.integrate(d, axis=0)
        # Single-cell axis: takes mean → value preserved, shape collapsed
